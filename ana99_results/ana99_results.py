...
cwd = '/home/physics/estszj/research_project/ana99_plotResults/'  # CHECK HERE
dir_home='/home/physics/estszj/research_project/'
dir_host_distr=os.path.join(dir_home, 'study_area/host_distributions')
dir_infection_visit=os.path.join(cwd,'infection_visit_count')
dir_metric=os.path.join(cwd, f'output_metric_sfreq{SURVEY_FREQ}_nsite{NSITES}')
dir_config=os.path.join(cwd, 'output_config')
dir_trace=os.path.join(cwd, 'output_trace')

dir_data_output=os.path.join(cwd, 'output_data')
filename_save = f"sfreq{SURVEY_FREQ}_nsite{NSITES}"
print(filename_save, flush=True)

"""Read distributions ========================================="""
distr_list = [
    load(os.path.join(dir_host_distr,f'area0{i}.joblib')) for i in range(1,9)
]

"""Read infection visit count ========================================="""
visit_count_list = load(os.path.join(cwd, 'infection_visit_count', 'infection_visit_count_8areas.joblib'))
...
for i in tqdm(range(len(area_road_perm))):
    host_distr_i = int(area_road_perm[i,0][-1])-1  #take the last string of the area name, convert to integer, minus 1
